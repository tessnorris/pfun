...
function generateOrchestrator(args, relFiles, rootUp) {
  ...
  if (args.mode === 'compile') {
    const rtRel = args.runtime ? path.relative(repoRoot, path.resolve(args.runtime)) : '';
    L.push(`export PFUN_RUNTIME=${rtRel ? '"$PROJECT_ROOT/' + rtRel + '"' : '""'}`);
    const ioRel = rtRel ? path.join(path.dirname(rtRel), 'pfun-io.js') : '';
  ...
  if (args.mode === 'interp') {
    L.push(`  ${timeoutPrefix}${pfunCmd} "$f"`);
    L.push('  local rc=$?');
  } else {
    L.push('  local base; base="$(basename "$f" .pf)"');
    L.push('  local outdir; outdir="$(mktemp -d)"');
    L.push(`  ${pfunCmd} -c "$f" -o "$outdir" || { echo "COMPILE FAILED: $f"; fail=$((fail+1)); failed_files+=("$f (compile)"); return; }`);
