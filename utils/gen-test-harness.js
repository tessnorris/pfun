...
function generateOrchestrator(args, relFiles) {
  ...
  L.push('set -u');
  L.push('cd "$(dirname "${BASH_SOURCE[0]}")" || exit 2');
  if (args.mode === 'compile') {
    L.push(`export PFUN_RUNTIME=${args.runtime ? shQuote(path.resolve(args.runtime)) : '""'}`);
    const ioGuess = args.runtime ? path.join(path.dirname(path.resolve(args.runtime)), 'pfun-io.js') : '';
    L.push(`export PFUN_IO=${ioGuess ? shQuote(ioGuess) : '""'}`);
  }
  L.push('');
  L.push('pass=0'); L.push('fail=0'); L.push('failed_files=()');
  L.push('');
  L.push('run_one() {');
  L.push('  local f="$1"');
  L.push('  echo'); L.push('  echo "=== $f ==="');
  if (args.mode === 'interp') {
    L.push(`  ${timeoutPrefix}${args.pfun} "$f"`);
    L.push('  local rc=$?');
  } else {
    L.push('  local base; base="$(basename "$f" .pf)"');
    L.push('  local outdir; outdir="$(mktemp -d)"');
    L.push(`  ${args.pfun} -c "$f" -o "$outdir" || { echo "COMPILE FAILED: $f"; fail=$((fail+1)); failed_files+=("$f (compile)"); return; }`);
    L.push('  local js; js="$(find "$outdir" -name "${base}.js" ! -path "*/lib/*" | head -1)"');
    L.push('  if [ -z "$js" ]; then echo "NO OUTPUT JS: $f"; fail=$((fail+1)); failed_files+=("$f (no js)"); return; fi');
    L.push('  local jsdir; jsdir="$(dirname "$js")"');
    ...
    L.push('    [ -n "$PFUN_IO" ] && { cp "$PFUN_IO" "$jsdir/lib/pfun-io.js" 2>/dev/null || true; cp "$PFUN_IO" "$outdir/lib/pfun-io.js" 2>/dev/null || true; }');
    L.push('  fi');
    L.push(`  ${timeoutPrefix}node "$js"`);
    L.push('  local rc=$?');
    L.push('  rm -rf "$outdir"');
  }
  L.push('  if [ "$rc" -eq 0 ]; then pass=$((pass+1));');
  L.push('  else fail=$((fail+1)); failed_files+=("$f (exit $rc)"); fi');
  L.push('}');
  L.push('');
  for (const f of relFiles) L.push(`run_one ${shQuote(f)}`);
  L.push('');
  L.push('echo');
  L.push('echo "========================================"');
  L.push('echo "Test files: $pass passed, $fail failed"');
  L.push('if [ "$fail" -ne 0 ]; then');
  L.push('  echo; echo "Failed:"');
  L.push('  for ff in "${failed_files[@]}"; do echo "  - $ff"; done');
  L.push('  exit 1');
  L.push('fi');
  L.push('echo "ALL TEST FILES PASSED"');
  L.push('exit 0');
  L.push('');
  return L.join('\n');
}
...
function main() {
  ...
  if (args.orchestrate) {
    const outDir = path.dirname(path.resolve(args.orchestrate));
    const rel = generated.map(f => path.relative(outDir, path.resolve(f)) || path.basename(f));
    fs.writeFileSync(args.orchestrate, generateOrchestrator(args, rel), { mode: 0o755 });
