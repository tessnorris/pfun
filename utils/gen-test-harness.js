...
function ensureImports(lines, needed) {
  // Insert missing imports after the final complete import declaration. Named
  // import lists may span several lines, so the last line that starts with
  // `import` is not necessarily a safe insertion point.
  ...
  let importStart = -1;
  for (let i = 0; i < lines.length; i++) {
    if (importStart < 0 && /^\s*import\b/.test(lines[i])) importStart = i;
    if (importStart >= 0 && /;\s*(?:\/\/.*)?$/.test(lines[i])) {
      lastImport = i;
      importStart = -1;
    }
  }
  if (importStart >= 0) lastImport = importStart;
...
function generateOrchestrator(args, relFiles, rootUp) {
  ...
  L.push(`# mode=${args.mode} files=${relFiles.length}`);
  L.push('set -u');
  L.push('');
