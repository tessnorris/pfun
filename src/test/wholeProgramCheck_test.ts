...
describe('Whole-program static checker (checkProgram) — Stage 1: graph + purity, Stage 2: types + exhaustiveness', () => {
  ...
  describe('Stage 3: all-errors batching across the whole graph', () => {
    function checkAll(fixtureName: string) {
      const entry  = path.join(FIXTURES, fixtureName);
      const loader = new ModuleLoader(FIXTURES);
      registerBuiltinModules(loader);
    ...
    it('collects a SEPARATE purity violation from each of two independent, unrelated leaf modules in one run, not just the first', () => {
      // batch_purity_leaf_a.pf misuses a proc as a value; batch_purity_leaf_b.pf
      // mutates a var from a function — two different rules, two different
      // files, neither depending on the other. Both must be reported from a
      // single checkProgram() call, not just whichever is visited first.
      const { errors } = checkAll('batch_purity_entry.pf');
      expect(errors.length).toBe(2);
      expect(errors[0].pfunMessage).toContain('batch_purity_leaf_a.pf');
      expect(errors[0].pfunMessage).toContain("Functions cannot use 'sideEffectA' as a value");
      expect(errors[1].pfunMessage).toContain('batch_purity_leaf_b.pf');
      expect(errors[1].pfunMessage).toContain("Functions cannot mutate 'counterB'");
    });

    it('orders batched errors dependency-first, matching the topological order the graph is walked in (leaves before the entry that imports them)', () => {
      // batch_purity_leaf_a.pf is imported before batch_purity_leaf_b.pf in
      // batch_purity_entry.pf's own source — dependency-first order means
      // leaf_a's error should appear before leaf_b's, mirroring
      // buildModuleGraph's documented post-order guarantee.
      const { errors } = checkAll('batch_purity_entry.pf');
      const aIdx = errors.findIndex(e => e.pfunMessage.includes('batch_purity_leaf_a.pf'));
      const bIdx = errors.findIndex(e => e.pfunMessage.includes('batch_purity_leaf_b.pf'));
      expect(aIdx).toBeGreaterThanOrEqual(0);
      expect(bIdx).toBeGreaterThanOrEqual(0);
      expect(aIdx).toBeLessThan(bIdx);
    });

    it('collects a SEPARATE type error from each of two independent leaf modules in one run', () => {
      const { errors } = checkAll('batch_type_entry.pf');
      expect(errors.length).toBe(2);
      expect(errors[0].pfunMessage).toContain('batch_type_leaf_a.pf');
      expect(errors[1].pfunMessage).toContain('batch_type_leaf_b.pf');
      for (const e of errors) expect(e.pfunMessage).toContain('Cannot unify');
    });

    it("runs BOTH checkProcedureUsage and checkTypes on a single module even though the first one already failed it — a purity violation does not suppress that same module's type error", () => {
      // batch_both_checks_one_module.pf has a proc-as-value misuse AND an
      // unrelated unification failure in the same function. Both must
      // surface from one checkProgram() call over this one file.
      const { errors } = checkAll('batch_both_checks_one_module.pf');
      expect(errors.length).toBe(2);
      expect(errors.some(e => e.pfunMessage.includes('[Purity]'))).toBe(true);
      expect(errors.some(e => e.pfunMessage.includes('[TypeCheck]'))).toBe(true);
      // It's the entry file itself — no "In <path>:" header on either.
      for (const e of errors) expect(e.pfunMessage).not.toMatch(/^In .*batch_both_checks_one_module\.pf/m);
    });

    it("does NOT cascade a broken module's failure into false positives for a clean module that imports it (permissive fallback, not a second bogus error)", () => {
      // batch_no_cascade_lib.pf fails its OWN check (an internal proc-as-value
      // misuse, unrelated to anything batch_no_cascade_consumer.pf does).
      // batch_no_cascade_consumer.pf legitimately imports and calls its
      // `value` export — since the lib failed, that import resolves
      // permissively (kind 'other' / type Unknown, exactly like an
      // unresolvable import), so the consumer must NOT get a second,
      // spurious error of its own.
      const { errors, checkedAsts } = checkAll('batch_no_cascade_consumer.pf');
      expect(errors.length).toBe(1);
      expect(errors[0].pfunMessage).toContain('batch_no_cascade_lib.pf');
      expect(errors[0].pfunMessage).toContain("Functions cannot use 'sideEffect' as a value");
      // The failing module is withheld from checkedAsts (its info was never
      // safe to trust downstream); everything that itself passed — even
      // something that imports the failing module — still gets cached.
      expect(checkedAsts.has(path.join(FIXTURES, 'batch_no_cascade_lib.pf'))).toBe(false);
      expect(checkedAsts.has(path.join(FIXTURES, 'batch_no_cascade_consumer.pf'))).toBe(true);
    });

    it('`error` remains a convenience alias for `errors[0]`, so existing single-error callers see identical behavior', () => {
      const { error, errors } = checkAll('main_bad.pf');
      expect(errors.length).toBe(1);
      expect(error).toBe(errors[0]);
    });

    it('a graph-level failure (missing module) is still reported as exactly one error — there is no graph left to keep walking once resolution itself fails', () => {
      const { errors } = checkAll('main_missing.pf');
      expect(errors.length).toBe(1);
      expect(errors[0].pfunMessage).toContain('Module not found');
    });

    it('a graph-level failure (circular import) is still reported as exactly one error', () => {
      const { errors } = checkAll('cycle_a.pf');
      expect(errors.length).toBe(1);
      expect(errors[0].pfunMessage).toContain('Circular import detected');
    });

    it('a fully passing program still returns an empty errors array (not just a null error)', () => {
      const { error, errors } = checkAll('main_good.pf');
