...
printf 'βrest\r\nlast\r' | \
	PFUN_HOME="$ROOT" node "$STAGE1" \
		run spec/slice-d1/io-example.pf \
		> "$WORK/io.stdout.txt" \
...
diff -u \
	spec/slice-d1/stdout-expected.txt \
	"$WORK/io.stdout.txt"

diff -u \
	spec/slice-d1/stderr-expected.txt \
	"$WORK/io.stderr.txt"
...
set +e
PFUN_HOME="$ROOT" node "$STAGE1" \
	run spec/slice-d1/exit-status.pf \
...
PFUN_HOME="$ROOT" node "$STAGE1" \
	build spec/slice-d1/io-example.pf \
